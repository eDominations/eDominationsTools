...
<li><a href='/disaster'>Disasters</a></li>
        ...
        <body>
        <?php
$player= DB::Select("SELECT players.name,players.DMG1HIT, SUM(last7days.Hits) as Hits, players.MilitaryRank,players.Energy,players.Level,players.Strength,players.CS,players.MilitaryUnitID,players.MilitaryRank2,players.ID,mudetails.MilitaryUnit,players.TotalDMG, SUM(last7days.DMG) as Last7, SUM(last35days.DMG) as Last35
FROM players
INNER JOIN mudetails ON players.MilitaryUnitID = mudetails.ID
INNER JOIN last7days ON players.ID = last7days.ID2
INNER JOIN last35days ON players.ID = last35days.ID2
WHERE players.ID > 101
AND mudetails.ID = '$URI'
GROUP BY ID  
...
<div class="row"> ........</div>
  ...
  <div class="row"> ........</div>
    ...
    <thead class="thead-dark">
        <tr>
                <th colspan="12" ><h2 align="center">PLAYER LIST</h2></th>
                </tr>
...
."<td>".number_format($obj5->Last35)."</td>"
...
."<td>".number_format($obj5->Hits)."</td></tr>";?>
 ...
 <th > </th>
                ...
                <th scope="col">LAST 35 DAYS DMG</th>
